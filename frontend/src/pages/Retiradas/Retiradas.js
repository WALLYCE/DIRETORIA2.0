...
export default function Retiradas(){
    ...
    async function getDados(){
        setBackdrop(true)
      const result = await axios.post(`${process.env.REACT_APP_BASE_URL}/cliente_servico/cancelamentos/retiradas`, {data_inicial: dataInicial, data_final: dataFinal});
          ...
          callback({ chartWrapper }) {
            ...
            dados.map((item)=>{
             if(item.cidade === itemSelected && item.onu!=='-' ){
                console.log(item)
                array.push({id: item.codigo, nome: item.nome, cidade: item.cidade, onu: item.onu, os: item.os})
             }
            })
