...
function ChartVendasVendedores(props) {
    ...
    useEffect(()=>{
      ...
      const counts=[];
        ...
        jsons().then(async (counts)=> {
          const vendedores = await axios.get(`${process.env.REACT_APP_BASE_URL}/vendas/vendedores/ativos`);
          const mapped = Object.entries(counts).map(([k,v]) => [`${k}`,parseInt(`${v}`), `${v}`, props.color]);

          vendedores.data.map((item) =>{
          var aux = false;
          const nome = removeTag(item.nome)
            ...
            if(aux ===false){
                mapped.push([nome, 0, '0', props.color])
            }
          })     
    ...
    useEffect(()=> {
     setBackdrop(!backdrop)
     axios.post(props.url,{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then((result)=> {
...
export default ChartVendasVendedores;