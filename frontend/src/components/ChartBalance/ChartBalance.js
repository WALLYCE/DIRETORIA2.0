...
function ChartBalance(props) {
 ...
 useEffect(()=> {
  ...
  axios.post(`${process.env.REACT_APP_BASE_URL}/vendas/cidade`, { data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
   .then(result => {
      ...
      axios.post(`${process.env.REACT_APP_BASE_URL}/cancelamentos_cidade`,{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
          .then(resultado => {
            var cancels=[];
...
function calculaSaldo(vendas, cancelamentos){
  ...
  var arrayGrafic = [["Cidade", "Venda",  { role: "annotation", type: "string" }, "Cancelamento", { role: "annotation", type: "string" }, "Saldo", { role: "annotation", type: "string" }]]
   var somatorio = 0;
...
const handleFiltrarClick = () => {
   ...
   fetch(`${process.env.REACT_APP_BASE_URL}/vendas/cidade`, {
   method: 'POST',
   headers: {
     'Accept': 'application/json, text/plain, */*',
     'Content-Type': 'application/json'
   },
   body: JSON.stringify({ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
  }).then(res => res.json())
   .then(result => {
      setVendas(result);
      var counts=[];
        ...
        fetch('http://192.168.88.88:5000/cancelamentos_cidade', {
          method: 'POST',
          headers: {
            'Accept': 'application/json, text/plain, */*',
            'Content-Type': 'application/json'
