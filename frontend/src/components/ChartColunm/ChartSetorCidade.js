...
export default function ChartSetorCidade(){
    ...
    async function getDados(){
        setBackdrop(true)
        const result = await axios.post(`${process.env.REACT_APP_BASE_URL}/setor/venda`, {data_inicial: dataInicial, data_final: dataFinal});
       ...
       return counts;
        ...
        const resultado = []
        for(var i=0; i<mappedInterna.length; i++){
          var aux = false;
            for(var k=0; k<mappedExterna.length; k++){
                if(mappedInterna[i][0] === mappedExterna[k][0]){
                  aux = true;
                  resultado.push([mappedInterna[i][0], mappedInterna[i][1], mappedInterna[i][1], mappedExterna[k][1],mappedExterna[k][1]])
                  mappedExterna.splice(k);
                  break;
                }
            }
            if(!aux){
              resultado.push([mappedInterna[i][0], mappedInterna[i][1],mappedInterna[i][1], 0,0])
            }
        }


        for(var i=0; i<mappedExterna.length; i++){
          var aux = false;
            for(var k=0; k<mappedInterna.length; k++){
                if(mappedInterna[k][0] === mappedExterna[i][0]){
                  aux = true;
                  break;
                }
            }
            if(!aux){
              resultado.push([mappedExterna[i][0], 0,0, mappedExterna[i][1], mappedExterna[i][1]])
            }
        }
      ...
      const chartEvents = [
        {
          ...
          callback({ chartWrapper }) {
            ...
            dados.map((item)=> {
              if(item.cidade === RowSelected && item.setor === ColSelected){
                modal.push({id: item.codigo, nome: item.nome , vendedor: item.vendedor, data: item.data, plano: item.plano, setor: item.setor})
              }
            })
