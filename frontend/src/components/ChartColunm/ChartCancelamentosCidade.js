    ...
    useEffect(()=>{
      ...
      const counts=[];
        ...
        jsons().then(async (counts)=> {
          const vendedores = await axios.get(`${process.env.REACT_APP_BASE_URL}/cidades/atendidas`);
          const mapped = Object.entries(counts).map(([k,v]) => [`${k}`,parseInt(`${v}`), `${v}`, props.color]);

          vendedores.data.map((item) =>{
          var aux = false;

            for(var i=0; i<mapped.length; i++){
                if(mapped[i][0] === item.nome){
                    aux = true; 
                    break;
                }
            }
            if(aux ===false){
                mapped.push([item.nome, 0, '0', props.color])
            }
          })     
          mapped.unshift([ "Cidade", "Quantidade", { role: "annotation", type: "string" }, {role: "style" }])
