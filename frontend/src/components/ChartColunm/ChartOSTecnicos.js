...
import ModalOSMapRealizadaTecnico from "../ModalOSMapRealizadaTecnico/ModalOSMapRealizadaTecnico";

function ChartOSTecnicos() {
    ...
    useEffect( ()=>{
      ...
      const counts=[];
        ...
        jsons().then(async (counts)=> {
          ...
          const mapped = Object.entries(counts).map(([k,v]) => [`${k}`,parseInt(`${v}`), `${v}`]);
          mapped.unshift(['Tecnico', "Quantidade", { role: "annotation", type: "string" }])
    ...
    useEffect(()=> {
      ...
      axios.post(`${process.env.REACT_APP_BASE_URL}/os/realizadas/map`, { data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(result => {
      ...
      const chartEvents = [
        {
          ...
          callback({ chartWrapper }) {
            ...
            const counts=[];
            (dados && dados.forEach((x) => {
              if(x.tecnico_responsavel === itemSelected){
  ...
  return (
     ...
     <ModalOSMapRealizadaTecnico status={modalStats} label={labelModal} dados={dadosModal} event={()=>{setModalStats(false)}}/>
...
export default ChartOSTecnicos;