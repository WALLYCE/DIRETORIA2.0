...
import ModalOSCidade from "../ModalOSCidade/ModalOSCidade";
...
function ChartOSTipo() {
    ...
    useEffect(()=> {
      ...
      axios.post(`${process.env.REACT_APP_BASE_URL}/os/manutencao/clientes/map`, { data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(result => {
  ...
  return (
     ...
     <ModalOSCidade status={modalStats} label={labelModal} dados={dadosModal} event={()=>{setModalStats(false)}}></ModalOSCidade>
